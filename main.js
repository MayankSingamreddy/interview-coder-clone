...
async function captureScreenshot() {
  try {
    ...
    mainWindow.showInactive(); // Use showInactive instead of show
    if (mainWindow.webContents) {
...
async function processScreenshots() {
  try {
    ...
    const messages = [
      { type: "text", text: "Can you solve the question for me and give the final answer/code? Give a short high level explanation, then the full code, and a one line explanation of big o runtime, and space complexity." }
    ];
    ...
    // Ensure window is visible but doesn't steal focus
...
function resetProcess() {
  ...
  updateInstruction("S: Screenshot | A: Multi-mode");
}
...
function createWindow() {
  ...
  const y = height - 600; // Position at bottom of screen
  ...
  globalShortcut.register('CommandOrControl+Shift+A', async () => {
    try {
      if (!multiPageMode) {
        multiPageMode = true;
        updateInstruction("Multi-mode: A to add, S to finalize");
      }
      const img = await captureScreenshot();
      screenshots.push(img);
      updateInstruction("Multi-mode: A to add, S to finalize");
      
      // Make sure window is visible but doesn't steal focus
      if (!mainWindow.isVisible()) {
        mainWindow.showInactive();
      }
    } catch (error) {
